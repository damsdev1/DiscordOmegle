...
client.on('voiceStateUpdate', (oldState, newState) => {
    if (newState.channel){
        if (newState.channel.id == config.waitChannel_ID) {
                ...
                if(channels.size == 0){
                    ...
                    for (const [channelID, channel] of channels) {
                        ...
                        if(channel.members.size == 1){
                            for (const [memberID_move, member_move] of newState.channel.members) {
                                for( const [memberID, member] of channel.members){

                                    // Verify if the member will be moved and member already in channel have already matched
                                    if(dbcache_verify_match(memberID, memberID_move)){
                                        // Next Channel Check
                                        continue;
                                    } else {
                                        dbcache_add_match(memberID, memberID_move);
                                        member_move.voice.setChannel(channel).catch(err => {console.log(err)});
                                        break move;
                                    }
                                }
                            }
